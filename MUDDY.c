		...
		for ( i = 0; i < m; ++i )
			for ( j = 0; j < n; ++j ) {
				...
				if ( j == 0 || g[i][j-1] == '.' ) {
					...
					lst[HOR][len[HOR]++] = (enc(i,j)|(enc(i,k)<<12));
				}
				if ( i == 0 || g[i-1][j] == '.' ) {
					for ( k = i+1; k < m && g[k][j] == '*'; ++k ) ;
					lst[VER][len[VER]++] = (enc(i,j)|(enc(k,j)<<12));
				}
			}
		for ( i = 0; i < len[HOR]; deg[i++] = 0 ) ;
		for ( i = 0; i < len[HOR]; ++i ) 
			if ( length(lst[HOR][i]) )
				for ( j = 0; j < len[VER]; ++j )
					if ( fully_contained(lst[HOR][i],lst[VER][j]) ) 
						++deg[i];
		adj = (int **)malloc(len[HOR]*sizeof *adj);
		for ( i = 0; i < len[HOR]; ++i )
			if ( deg[i] )
				adj[i] = (int *)malloc(deg[i]*sizeof *adj[i]);
		for ( i = 0; i < len[HOR]; ++i ) 
			if ( length(lst[HOR][i]) )
				for ( deg[i] = 0, j = 0; j < len[VER]; ++j )
					if ( fully_contained(lst[HOR][i],lst[VER][j]) ) 
						adj[i][deg[i]++] = j+len[HOR];
		bpm();
		for ( i = 0; i < len[HOR]; ++i )
			if ( deg[i] )
				free(adj[i]);
