...
int seen[Q],yes,m,n,cnt,
	...
	dx[] = {-1,-1,0,1,1,1,0,-1},
	dy[] = {0,1,1,1,0,-1,-1,-1};
unsigned int q[QS],head,tail,d[Q],ans,dist[N][N];
...
int main() {
	...
	for ( scanf("%d",&ts); ts-- && 2 == scanf("%d %d",&m,&n); ) {
		...
		for ( x = 0; x < m; ++x )
			for ( y = 0; y < n; ++y )
				dist[x][y] = +oo;
		for ( ans = +oo, ++yes, x=(src&MASK(SH)),y=(src>>SH), dist[x][y]=0, head=tail=0, cnt=0, t = 0; t < 8; ++t ) {
			d[u=enc(x,y,t)] = 0, seen[u] = yes;
			nx=x+dx[t], ny=y+dy[t];
			assert( nx != x || ny != y );
			if ( vc(nx,ny) && g[nx][ny] != 'X' )
				d[u=enc(nx,ny,t)]=1, seen[u]=yes, ADD_TO_REAR(u);
		}
		while ( cnt ) {
			u = q[head], head=NEXT(head), --cnt, x=(u&MASK(SH)),y=((u>>SH)&MASK(SH));
			if ( e(x,y) == sink ) {
				if ( ans > d[u] )
					ans = d[u];
				continue ;
			...
			if ( vc(nx,ny) && g[nx][ny] != 'X' && (seen[v=enc(nx,ny,t)] != yes || d[v]>d[u]) )
				seen[v] = yes, d[v]=d[u], ADD_TO_FRONT(v);
			...
			for ( i = 0; i < len[t]; ++i ) {
				...
				if ( vc(nx,ny) && g[nx][ny] != 'X' && (seen[v=enc(nx,ny,j)] != yes || d[v]>d[u]+1) )
					seen[v] = yes, d[v]=d[u]+1, ADD_TO_REAR(v);
			}
		}
		if ( ans < +oo )
			printf("%u\n",ans);
